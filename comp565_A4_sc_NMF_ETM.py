...
######## Q5 plot t-SNE for NMF-Poisson and scETM ########
...
colrs = ["royalblue", "orange", "green", "red", "blueviolet", "brown", "hotpink", "darkkhaki", "turquoise", "lightsteelblue", "peachpuff", "lightgreen", "pink"]
lut = dict(zip(mp_anndata.obs["Celltype"].unique(), colrs))
...
for i in range(16):
    ...
    result = result.append(df_top5, ignore_index=True, sort=False)


result = result.fillna(0)
result = result.set_index('genes')

fig, ax = plt.subplots(figsize=(10, 33))
ax = heatmap(result, vmax=0.2, cmap="Reds", linecolor="white")
